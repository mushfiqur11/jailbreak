...
class TracebackModule:
    ...
    def __init__(
        ...
        self.retry_on_parse_failure = bool(retry_on_parse_failure)
        self.max_parse_retries = max(1, int(max_parse_retries))
        
        logger.info(
            "TracebackModule initialized with min_turns=%s, retry_on_parse_failure=%s, max_parse_retries=%s",
            min_turns,
    ...
    def perform_traceback(
        ...
        # Call reasoning agent to identify fallback turn; fail-soft if parsing remains unstable
        try:
            traceback_analysis = self.reasoning.perform_informed_traceback(
                goal=goal,
                target_safety_prompt=target_safety_prompt,
                conversation_str=conversation_str,
                max_parse_retries=self.max_parse_retries if self.retry_on_parse_failure else 1,
            )
            ...
            heuristic_turn = max(1, len(conversation_history) - 2)
            fallback_turn = heuristic_turn
            failure_reason = (
                "Traceback parse fallback applied due to reasoning output parse failure: "
                f"{e}"
            )
            logger.warning(
                "Traceback parsing failed; using heuristic fallback turn=%s for goal. Error=%s",
                heuristic_turn,
                e,
            )
...
def create_traceback_module(reasoning_agent, config: Dict[str, Any]) -> TracebackModule:
    ...
    retry_on_parse_failure = settings.get("retry_on_parse_failure", True)
    max_parse_retries = settings.get("max_parse_retries", 3)
