...
Implements the ATTCK-R architecture with:
...
def configure_logging(config) -> None:
    """
    Configure logging from config/env with safe fallback.

    Priority:
      1) LOG_LEVEL environment variable (when non-empty)
    ...
    config_level = str(get_config_value(config, 'logging', 'level', default='INFO')).strip()

    if env_level:
        chosen_level_name = env_level.upper()
        level_source = 'env:LOG_LEVEL'
    else:
        chosen_level_name = config_level.upper() if config_level else 'INFO'
        level_source = 'config.logging.level'

    chosen_level = getattr(logging, chosen_level_name, None)
    if not isinstance(chosen_level, int):
        chosen_level_name = 'INFO'
        chosen_level = logging.INFO
        level_source = f'invalid({level_source}) -> fallback'
    ...
    logging.getLogger(__name__).info(
        "Logging configured at level=%s (source=%s)",
        chosen_level_name,
        level_source,
    )
...
class PipelineOrchestrator:
    ...
    def __init__(self, config):
        ...
        self.save_every_n_goals = get_config_value(config, 'pipeline', 'save_every_n_goals', default=5)

        # Fail fast on API/signature drift between agents and prompt builders
        self._run_contract_preflight()
        ...
        logger.info(f"  Checkpoint save frequency: every {self.save_every_n_goals} goals")

    def _run_contract_preflight(self) -> None:
        """Run strict interface checks to catch signature drift before runtime."""
        from jailbreak.agentic_module.agents.reasoning_prompts import get_informed_traceback_prompt

        sig = inspect.signature(get_informed_traceback_prompt)
        required = ["goal", "conversation_str"]
        optional = ["target_safety_prompt"]

        missing_required = [p for p in required if p not in sig.parameters]
        missing_optional = [p for p in optional if p not in sig.parameters]
        if missing_required or missing_optional:
            raise RuntimeError(
                "Prompt contract mismatch for get_informed_traceback_prompt. "
                f"Expected parameters include required={required} and optional={optional}. "
    ...
    def run(self) -> None:
        ...
        for idx, goal_text in enumerate(goals):
            ...
            if self.save_every_n_goals and (idx + 1) % self.save_every_n_goals == 0:
                logger.info(f"💾 Checkpoint save after {idx + 1} processed goals")
