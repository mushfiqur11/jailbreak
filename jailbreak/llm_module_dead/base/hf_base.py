...
class HuggingFaceBase(BaseLLM):
    ...
    def _setup_model_path(self) -> str:
        ...
        # Check if model exists locally and appears complete enough to load.
        def _has_minimum_model_files(path: str) -> bool:
            ...
            # Minimal config + any plausible weights/tokenizer/processor artifact.
            has_config = "config.json" in names
            has_weights = any(
                n.endswith(".safetensors") or n in {"pytorch_model.bin", "model.safetensors"}
                for n in names
            )
            ...
            return has_config and (has_weights or has_tokenizer_or_processor)

        needs_download = (
            self.config.get("redownload", False)
            or not _has_minimum_model_files(model_path)
        )
    ...
    def _load_tokenizer(self) -> None:
        ...
        try:
            ...
            err_msg = str(e)
            needs_slow_fallback = (
                "protobuf" in err_msg.lower()
                or "importerror" in err_msg.lower()
                or "NoneType' object has no attribute 'endswith" in err_msg
            )
            ...
            print(
                "⚠️ Fast tokenizer load failed (likely protobuf/fast-tokenizer conversion issue). "
    ...
    def forward(self, messages: Optional[List[Dict[str, str]]] = None) -> Dict[str, Any]:
        ...
        # Build attention mask explicitly (important when pad_token_id == eos_token_id)
        # to avoid generation warnings and unreliable behavior.
        if self.tokenizer.pad_token_id is not None:
            attention_mask = (tokenized_input != self.tokenizer.pad_token_id).long()
        else:
