...
# ============================================
# SHORT V3 MASTER LAUNCHER
# Launches V3 short 5-stage chained experiments from experiments_v3_short.
# Supports optional omission list for experiments to skip.
# ============================================

echo "============================================"
echo "JAILBREAK EXPERIMENT LAUNCHER SHORT V3"
echo "============================================"
echo "Date: $(date)"
echo ""
...
EXPERIMENTS_DIR="configs/experiments_v3_short"
EXPERIMENT_LIST="$EXPERIMENTS_DIR/experiment_list.txt"

if [ ! -d "$EXPERIMENTS_DIR" ] || [ ! -f "$EXPERIMENT_LIST" ]; then
    echo "SHORT V3 configs/list missing. Generating now..."
    python generate_experiment_configs_v3_short.py
fi
...
while IFS= read -r exp_name || [[ -n "$exp_name" ]]; do
    ...
    echo "Submitting SHORT V3: $exp_name"
    JOB_ID=$(sbatch --job-name="${exp_name}_v3s" --export=CONFIG_NAME="$exp_name" run_experiment_v3_short.slurm 2>&1)
