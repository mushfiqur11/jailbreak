...
# ============================================
# V2 REST MASTER LAUNCHER
# Launches V2 REST 3-stage experiments from experiments_v2_rest folder.
# Supports optional omission list for experiments to skip.
# ============================================

echo "============================================"
echo "JAILBREAK EXPERIMENT LAUNCHER V2 REST"
echo "============================================"
echo "Date: $(date)"
echo ""
...
EXPERIMENTS_DIR="configs/experiments_v2_rest"
EXPERIMENT_LIST="$EXPERIMENTS_DIR/experiment_list.txt"

if [ ! -d "$EXPERIMENTS_DIR" ] || [ ! -f "$EXPERIMENT_LIST" ]; then
    echo "V2 REST configs/list missing. Generating now..."
    python generate_experiment_configs_v2_rest.py
fi
...
while IFS= read -r exp_name || [[ -n "$exp_name" ]]; do
    ...
    echo "Submitting V2 REST: $exp_name"
    JOB_ID=$(sbatch --job-name="${exp_name}_v2r" --export=CONFIG_NAME="$exp_name" run_experiment_v2.slurm 2>&1)
